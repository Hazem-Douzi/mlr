% =====================================================================
%  Modele 3 (Transfer Learning) -> Evaluation -> Comparaison -> Conclusion
%  Systeme Intelligent de Controle Qualite des Impellers
%  Filiere BDIA1 - Annee universitaire 2025-2026
%
%  NOTE : ce fichier est concu pour etre inclus via \input{} depuis
%         main.tex. Toute la configuration globale (packages, couleurs,
%         infobox, etc.) est deja definie dans main.tex.
% =====================================================================

% =====================================================================
%  CHAPITRE 4 - Modele 3 : Transfer Learning avec EfficientNetB0
% =====================================================================
\chapter{Modèle 3 : Transfer Learning avec EfficientNetB0}

\section{Principe du Transfer Learning}

Le \emph{Transfer Learning} (TL, apprentissage par transfert) consiste a \textbf{reutiliser un réseau de neurones déjà entraîne} sur une grande base d'images --- ici \textsc{ImageNet}, qui contient $\sim 1{,}2$ million d'images réparties sur $1\,000$ classes --- pour résoudre un nouveau problème plus spécifique. Ce paradigme exploite le fait que les premières couches d'un CNN apprennent des concepts visuels \emph{génériques} (contours, textures, formes) qui sont \emph{transférables} a presque toutes les tâches d'images naturelles.
...
\section{Stratégie en deux phases}
...
\section{Architecture choisie : EfficientNetB0}
...
\section{Source des données et preparation}
...
\begin{infobox}[Phase 2 - Résultats]
...
\section{Courbes d'entraînement consolidées}
...
\begin{itemize}
    ...
    \item L'AUC approche $0{,}99$~: pour deux images tirées au hasard (une défectueuse, une OK), le modèle les separe presque parfaitement --- performance excellente pour le contrôle qualité industriel.
\end{itemize}

% =====================================================================
%  CHAPITRE 5 - Evaluation finale et discussion
% =====================================================================
...
\begin{infobox}[Métriques industrielles clés - Phase 2]
...
En contrôle qualité industriel, les coûts associés aux faux négatifs (FN) et aux faux positifs (FP) sont profondément \emph{asymétriques}~:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Taille modeste}~: $\sim 5$ Mo après conversion en TFLite, deployable sur Edge devices (Raspberry Pi 4, Jetson Nano).
...
% =====================================================================
%  CHAPITRE 6 - Comparaison globale des trois modeles
% =====================================================================
\chapter{Comparaison globale des trois modèles}
...
\paragraph{Synthèse de la progression.}
La démarche en trois étapes met en évidence l'apport décisif du Transfer Learning~:
\begin{itemize}
    \item Le \textbf{MLP} (chapitre précédent) sert de baseline et plafonne à $\sim 68\%$ d'accuracy~: il ignore la structure spatiale des images et n'extrait que des relations globales pixel-à-pixel.
    \item Le \textbf{CNN} avec data augmentation atteint $\sim 95\%$ grâce à la convolution (invariance par translation, hierarchie de features) et à l'augmentation qui élimine le sur-apprentissage.
    \item L'\textbf{EfficientNetB0 + Transfer Learning} pousse les performances à $99{,}44\%$ avec un coût d'entraînement très réduit, validant définitivement le choix d'un backbone pré-entraîne sur ImageNet.
\end{itemize}

\paragraph{Lecture verticale du tableau.}
Au-delà de l'accuracy, le tableau met en lumière trois indicateurs souvent négligés~: le recall sur la classe défectueuse (la métrique critique en QC industriel), la présence ou non de sur-apprentissage, et le coût d'entraînement. C'est sur ce dernier point que le Transfer Learning tire son avantage le plus stratégique~: la phase 1 entraîne moins de $1\,300$ paramètres, ce qui permet une convergence très rapide même sans GPU haut de gamme.
...
\begin{itemize}
    \item \textbf{Pipeline cross-framework}~: implémentation double TensorFlow et PyTorch, garant de la reproductibilité.
...
\begin{itemize}
    \item \textbf{Deploiement embarque}~: quantification \texttt{int8} et export vers TFLite pour exécution sur Raspberry Pi, Jetson Nano ou Edge TPU en bord de ligne. Reduction mémoire et accélération sans perte significative de performance.
